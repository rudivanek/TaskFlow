...
- Status row tinting: each row receives a subtle background tint matching its task status (green=Done, amber=In Review, red=Blocked; no tint for unset/other statuses). **Doing / In Progress** rows use a soft purple highlight: background #F5F3FF (custom `violet-70` theme color, equivalent to Tailwind violet-50) with a 3px solid #7C3AED (Tailwind violet-600) left accent border, applied via the `status-doing-row` CSS class so the status reads instantly despite the subtle fill. Text color stays dark/black for readability. A smooth `transition-colors duration-150` applies. In dark mode (`prefers-color-scheme: dark`), the background switches to `rgba(139, 92, 246, 0.22)` so the highlight stays visible without washing out the text. When a Doing row is also the target of a dependency-hover, the blue dependency-hover highlight takes precedence and the purple Doing tint is suppressed. The status-to-color mapping is shared with the Kanban view (see "Kanban card status tinting" below) via a single `getStatusRowStyles()` helper so future changes only need to be made in one place.
- **Kanban card status tinting**: Kanban task cards mirror the exact same status-based background colors the task grid rows use, applied via the shared `getStatusRowStyles()` helper. The background tint is applied to the card surface itself, and the 3px solid #7C3AED (violet-600) left accent border for Doing/In Progress cards overrides the card's date-based left border. Text color stays dark/black. A smooth `transition-colors duration-150` applies. In dark mode, the same dark-mode values as the grid are used (for Doing: `rgba(139, 92, 246, 0.22)`). The card's existing border radius, shadow, padding, spacing, content, and drag-and-drop behavior are unchanged. Column headers and column backgrounds are untouched.
...
- **Two-way status sync between main tasks and subtasks**: All status changes are routed through a single central handler (`useStatusSync` hook → `changeTaskStatus(taskId, newStatusId, { source })`). Every status write path (grid dropdown, Kanban drag-and-drop, task detail modal, mobile views) calls this handler. Sync-initiated changes use `source: 'sync'` and never open a modal, preventing loops.
  - **Downward sync (main task → subtasks)**: When the user changes a main task's status, the main task is saved immediately. If the task has subtasks and at least one differs from the new status, a confirmation modal appears: "Update subtasks too? Set {N} of {M} subtasks to '{New Status}'?" with a scrollable list of subtasks that will change (name + current status → new status). Buttons: [No, only main task] / [Yes, update subtasks]. Closing the modal (X, Esc, click outside) = No. If all subtasks already match or there are no subtasks, no modal appears. Subtask changes are saved in a single Supabase batch call (`batchUpdateSubtaskStatus` using `.in('id', ids)` with `.select()` to verify the returned row count matches the number of IDs sent; if not, an error toast is shown). After a successful cascade, the hook broadcasts a `subtaskRefresh` event (with a nonce) that forces every `SubtaskList` instance for that task to re-fetch from the database, so the UI updates immediately in both the Grid expanded rows and the Kanban detail modal without a page reload.
