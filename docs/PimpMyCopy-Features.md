...
- Status row tinting: each row receives a subtle background tint matching its task status (green=Done, amber=In Review, red=Blocked; no tint for unset/other statuses). **Doing / In Progress** rows use a stronger purple highlight: background #DDD6FE (Tailwind violet-200) with a 3px solid #7C3AED (Tailwind violet-600) left accent border, applied via the `status-doing-row` CSS class so the status reads instantly without relying on the fill alone. Text color stays dark/black for readability. A smooth `transition-colors duration-150` applies. In dark mode (`prefers-color-scheme: dark`), the background switches to `rgba(139, 92, 246, 0.35)` so the highlight stays visible without washing out the text. When a Doing row is also the target of a dependency-hover, the blue dependency-hover highlight takes precedence and the purple Doing tint is suppressed.
